...
function processLine(time) {
    ...
    var reindeerPath = "M0,20 L5,15 L8,18 L10,10 L12,5 L15,8 L14,12 L18,10 L22,12 L20,15 L25,14 L30,16 L28,20 L32,22 L30,28 L25,26 L20,28 L15,26 L10,28 L5,25 L0,28 Z M12,3 L14,0 L16,4 M16,3 L18,0 L19,4";
    
    // Sleigh silhouette
    var sleighPath = "M0,15 Q5,5 15,5 L45,5 Q55,5 55,15 L55,25 Q55,35 45,35 L10,35 Q0,35 0,25 Z M5,35 Q0,40 5,45 L15,45 Q20,40 15,35 M40,35 Q35,40 40,45 L50,45 Q55,40 50,35";
    
    // Santa in sleigh (simplified silhouette)
    var santaPath = "M20,5 Q25,-5 30,5 L35,5 Q38,0 35,5 L35,15 L15,15 L15,5 Z M22,15 L22,25 L32,25 L32,15 M18,25 L18,30 L35,30 L35,25";
    
    // Star path
    var starPath = "M0,-10 L2,-3 L10,-3 L4,2 L6,10 L0,5 L-6,10 L-4,2 L-10,-3 L-2,-3 Z";
    ...
    var stars = [];
    var starPositions = [
        {x: 50, y: 50}, {x: 150, y: 80}, {x: 250, y: 40}, {x: 350, y: 100},
        {x: 450, y: 60}, {x: 550, y: 90}, {x: 650, y: 45}, {x: 750, y: 75},
        {x: 100, y: 150}, {x: 300, y: 180}, {x: 500, y: 140}, {x: 700, y: 170},
        {x: 80, y: 250}, {x: 200, y: 220}, {x: 400, y: 260}, {x: 600, y: 230}
    ];
    
    for (var i = 0; i < starPositions.length; i++) {
        var twinkle = 0.5 + Math.sin(time * 3 + i * 0.7) * 0.5;
        var starSize = 3 + Math.sin(time * 4 + i) * 1;
        stars.push('<View left="' + starPositions[i].x + '" top="' + starPositions[i].y + 
                   '" width="' + starSize + '" height="' + starSize + 
                   '" background-color="#ffffff" border-radius="50%" opacity="' + twinkle + '"/>');
    }
    ...
    for (var r = 0; r < 4; r++) {
        ...
        reindeer.push(
            '<View left="' + rx + '" top="' + (ry + bounce) + '" width="35" height="30" opacity="1">' +
                '<View width="35" height="30" background-color="#4a3728" path="' + reindeerPath + '"/>' +
            '</View>'
        );
    }
    ...
    for (var s = 0; s < 30; s++) {
        ...
        snowflakes.push('<View left="' + sx + '" top="' + sy + 
                        '" width="' + ssize + '" height="' + ssize + 
                        '" background-color="#ffffff" border-radius="50%" opacity="' + sopacity + '"/>');
    }
    ...
    for (var t = 0; t < treePositions.length; t++) {
        ...
        trees.push(
            '<View left="' + treeX + '" top="' + (groundY - treeHeight) + 
            '" width="40" height="' + treeHeight + '">' +
                '<View left="15" top="' + (treeHeight - 20) + '" width="10" height="20" background-color="#2d1810"/>' +
                '<View left="0" top="0" width="40" height="' + (treeHeight - 15) + 
                '" background-color="#1a472a" path="M20,0 L40,' + (treeHeight-15) + ' L0,' + (treeHeight-15) + ' Z"/>' +
            '</View>'
        );
    }
    ...
    if (time > 2 && time < 4) {
        ...
        hoText = '<Label left="' + (sleighX - 30) + '" top="' + (sleighY - 50) + 
                 '" font-size="24" color="#ffffff" opacity="' + hoOpacity + '" text="HO HO HO!"/>';
    }
    ...
    '<View width="100%" height="100%" background-color="#0a0a20">' +
        ...
        // Moon
        '<View left="650" top="30" width="80" height="80" opacity="' + moonGlow + '">' +
        ...
        '<View left="' + (reindeerLeadX + 30) + '" top="' + reinsY + '" width="' + (sleighX - reindeerLeadX - 25) + '" height="2" background-color="#3d2817" opacity="0.7"/>' +
        
        // Sleigh
        '<View left="' + sleighX + '" top="' + sleighY + '" width="60" height="50" rotate="' + sleighRotate + '">' +
            // Sleigh body
        ...
        // Ground snow
        '<View left="0" top="' + groundY + '" width="100%" height="80" background-color="#f0f8ff"/>' +
        '<View left="0" top="' + (groundY - 10) + '" width="100%" height="20" linear-gradient="rgba(240,248,255,0), #f0f8ff"/>' +
        
        // Trees
        trees.join('') +
        
        // Snow mounds
        '<View left="0" top="' + (groundY + 10) + '" width="120" height="40" background-color="#ffffff" border-radius="50%"/>' +
        '<View left="200" top="' + (groundY + 15) + '" width="80" height="30" background-color="#ffffff" border-radius="50%"/>' +
        '<View left="400" top="' + (groundY + 5) + '" width="150" height="50" background-color="#ffffff" border-radius="50%"/>' +
        '<View left="650" top="' + (groundY + 12) + '" width="100" height="35" background-color="#ffffff" border-radius="50%"/>' +
