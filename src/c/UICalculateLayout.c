...
static double _layoutNodes( UIStructure *node, UIRenderPosition *render_pos )  {
    ...
    UIRenderPosition *child_render_pos = UIRenderPosition_With( node->paddingLeft->pixels, node->paddingTop->pixels);
    ...
    if (node->direction->is_set && EQUALS(node->direction->s_value,"bottom-to-top")) {
        ...
        while(childNode) {
            ...
            if( childNode->calculated.lineBreak ||  (row_width > node->innerWidth->pixels && (row_width - node->innerWidth->pixels > 0.5))) {
                if(node->align->is_set && (EQUALS(node->align->s_value,"right") || EQUALS(node->align->s_value,"center"))) {
                    // align right
                    double deltaX = node->paddingLeft->pixels + node->innerWidth->pixels + 0.5*lineEnd->marginLeft->pixels - lineEnd->calculated.x - lineEnd->calculated.width;
        ...
        if(node->align->is_set && (EQUALS(node->align->s_value,"right") || EQUALS(node->align->s_value,"center"))) {
            ...
            if(lineStart && lineEnd) {
                ...
                double deltaX = node->paddingLeft->pixels + node->innerWidth->pixels + lastItem->marginLeft->pixels - lastItem->calculated.x - lastItem->calculated.width;
                if(node->align->is_set && (EQUALS(node->align->s_value,"center") )) { deltaX = deltaX / 2; } // align center
                row_item = lineStart;
                while(row_item) {
                    row_item->calculated.x += deltaX;
    ...
    if(!node->height->is_set) {
        ...
        if(strcmp(node->tagName, "Label") == 0) {
            ...
            if(node->text->is_set) {
                UITextDimensions *dim = calculateTextDimensions( node, node->innerWidth->pixels );
                if(dim) {
...
UIRenderPosition* calculateUIStructureLayout( UIStructure *this, UIStructure *parentNode, UIRenderPosition *render_pos  ) { 
    UIRenderPosition *newPOS = UIRenderPosition_With( render_pos->x,  render_pos->y );
    ...
    if( node->left->is_set ) {
        node->calculated.x = node->marginLeft->pixels + parentNode->paddingLeft->pixels + node->left->pixels;
        node->calculated.absolute = 1;
    } else {
        node->calculated.x = render_pos->x + node->marginLeft->pixels;
    }
    if( node->top->is_set ) {
        node->calculated.y = node->marginTop->pixels + parentNode->paddingTop->pixels + node->top->pixels;
        node->calculated.absolute = 1;
    } else {
