...
static void init_duktape() {
    ...
    if (duk_peval_file(ctx, "myFile.js") != 0) {
...
*/
    ...
    if(ui->fontSize->is_set) fontSize = ui->fontSize->f_value;
...
static void test_xml(char *filename, char *outputFileName) {
    ...
    calculateUIStructureLayout( node_tree, container, UIRenderPosition_With(0,0) );
...
static void test_duktape() {
    duk_context *ctx = NULL;
    
    char *line = "<Label>OK</Label>";
    char *resVal;
    char idx;
    int ch;
